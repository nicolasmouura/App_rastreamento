...
export default function App() {
  ...
  const [temGrupo, setTemGrupo] =
    useState(false);

  const [pulouGrupo, setPulouGrupo] =
    useState(false);

  const [verificandoGrupo, setVerificandoGrupo] =
    useState(false);

  const [tela, setTela] =
    useState('menu');

  const [familiares, setFamiliares] =
    useState(familiaresIniciais);
